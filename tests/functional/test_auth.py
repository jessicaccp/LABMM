...
def test_second_register_without_token_is_forbidden(client, db_tables):
    # Bootstrap first user
    client.post("/auth/register", json={
        "first_name": "Admin", "last_name": "User",
        "email": "admin@test.local", "password": "password123",
    })
    # Second register without token — JWT layer raises 401 before RBAC check
    resp = client.post("/auth/register", json={
...
def test_second_register_by_super_admin_succeeds(client, db_tables):
    ...
    assert resp.status_code == 201
    assert resp.get_json()["member"]["is_super_admin"] is False
